...
export function AdminAILogsContent() {
  ...
  return <div className="space-y-6">
    <Card><CardHeader className="flex flex-col items-start gap-4 sm:flex-row sm:justify-between"><div><CardTitle>Nhật ký AI</CardTitle><CardDescription>Hành vi của người dùng và lịch sử chatbot.</CardDescription></div><div className="flex w-full gap-2 sm:w-auto"><div className="relative min-w-0 flex-1 sm:flex-none"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><Input className="w-full pl-9 sm:w-72" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm kiếm log..." /></div><Button variant="outline" size="icon" className="shrink-0" onClick={() => void load()}><RefreshCcw className="h-4 w-4" /></Button></div></CardHeader></Card>
...
function Pager({ pagination, page, onPageChange, label }: { pagination: Pagination | null; page: number; onPageChange: (value: number) => void; label: string }) {
  ...
  return <div className="flex flex-col gap-3 border-t px-4 py-4 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between sm:px-6"><span>Trang {pagination.page}/{pagination.totalPages} · {pagination.total} {label}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={page <= 1} onClick={() => onPageChange(page - 1)}>Trước</Button><Button size="sm" variant="outline" disabled={page >= pagination.totalPages} onClick={() => onPageChange(page + 1)}>Sau</Button></div></div>;
}
